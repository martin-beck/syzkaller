...
func (ctx *context) fmtCallBody(call prog.ExecCall, initCall bool) string {
	...
	for i, arg := range call.Args {
		if replacement, ok := ctx.sourceDialect().rewriteArgument(call, callName, i); ok {
			argsStrs = append(argsStrs, replacement)
			continue
		}
		...
		case prog.ExecArgConst:
			...
			pointerOffset := ""
			if arg.IsPointer {
				pointerOffset = ctx.sourceDialect().pointerOffset(arg.Value)
			}
			argsStrs = append(argsStrs, com+handleBigEndian(arg, ctx.constArgToStr(arg, native))+pointerOffset)
